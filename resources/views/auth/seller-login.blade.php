                ...
                <div class="px-3 pb-3">
                    <form id="loginForm" class="form-horizontal m-t-20">
                        ...
                        <div id="otpFieldContainer" class="form-group row" style="display: none;">
                            <div class="col-12"><input class="form-control" type="number" name="otp" required
                                    placeholder="OTP" disabled></div>
                            @error('otp')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            loginForm.addEventListener('submit', function(e) {
                ...
                const url = `{{ env('API_URL') . '/v1/login' }}`;
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.data.token) {
                            ...
                            let redirectUrl = `{{ env('APP_URL') . 'seller-details/${data.data.user.id}' }}`;
                            window.location.href = redirectUrl;
                        } else if (data.message === "OTP sent successfully") {
                            const inputValue = valueInput.value;
                            const html = `<div class="input-group">
                            <input class="form-control" type="number" name="value" required placeholder="Mobile Number" value="${inputValue}" disabled>
        ...
        function handleEditButtonClick() {
            ...
            const html =
                `<input class="form-control" type="number" name="value" required placeholder="Mobile Number" value="${inputValue}">`;
